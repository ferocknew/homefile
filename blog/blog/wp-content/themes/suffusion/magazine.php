			...
			foreach ($queries as $query) {
				if (isset($query->posts) && is_array($query->posts)) {
					...
					while ($query->have_posts()) {
						...
						else if ($ctr%$suf_mag_excerpts_per_row == $suf_mag_excerpts_per_row - 1 || ($ctr == $num_results - 1 && $ctr < $suf_mag_excerpts_per_row)) {
							$class = 'last';
						}
						else {
							$class = 'middle';
		...
		if ($categories != null && is_array($categories) && count($categories) > 0) {
			...
			if (is_array($categories)) {
				...
				foreach ($categories as $category) {
					...
					else if ($ctr%$suf_mag_catblocks_per_row == $suf_mag_catblocks_per_row - 1 || ($ctr == $num_results - 1 && $ctr < $suf_mag_catblocks_per_row)) {
						$class = 'last';
					}
					else {
						$class = 'middle';
