...
'//---------------------------------------------------------------------------
'//ASP 模板引擎 (Taihom.Template.class v3.0)
'//Code By Taihom
'//Taihom 原创
'//版权说明
...
Private strAuthor,strVersion,authorEmail
...
Private DIC_BLOCK_ATTR,DIC_BLOCK_LOOP_VAL,DIC_BLOCK_IF,DIC_BLOCK_LOOP,DIC_BLOCK_LOOP_LIST,DIC_BLOCK_LOOP_ATTR'内置变量
...
Private Sub Class_Initialize()
  ...
  strAuthor = "Taihom"
  strVersion = "3.0"
  authorEmail= "taihom@163.com"'本类作者的邮箱，如果您在使用过程中有不明白您可以来信告知
  ...
  intCacheTplTime = 10'模板缓存时间
  
  '以下信息请不要更改除非你对本类的思路跟实现比较了解否则请不要更改
  PREFIX = "TaihomTemplate"'这可不能更改，除非你一个站使用多个该模板类
  OPS = "==|<=|&lt;=|>=|&gt;=|>|&gt;|<|&lt;|!=|<>|&lt;&gt;|%"'默认支持的运算符
...
Private Function LoadFile(ByVal strFilePath,ByVal strCharset)
  If FSO.FileExists(strFilePath) = False Then LoadFile = Replace(strFileLoadFail,"@path",strFilePath) : Exit Function End If
...
Private Function LoadTemplate()
  Dim html,tplpath
  '加载模板
  If Len(strTemplate_path)=0 Then
	html = strTemplate'否则直接读取内存
  Else
	tplpath = Replace(strSITEROOT & strTemplate_dir & strTemplate_path,"/","\")
	html = LoadFile(tplpath,strCharset)''读取模板数据
  End If

  html = LoadInclude(html,tplpath)'include优先:LoadInclude(代码,来源路径)
  
  If Len(html)=0 Then Exit Function
  
  '把模板存储到临时变量
  '输出的模板是否带绝对路径
  strTemplate = iif(cbool(intAbsPath),AbsPath(html),html)
End Function
...
Private Function RelPath(ByVal strPath)
  ...
  Dim Matches
      EX.Pattern = "^(\.\.\/)+"
  Set Matches = EX.Execute(strPath)
  If Matches.count=0 Then
  	RelPath = Replace(strTemplate_dir,"\","/") & strPath
 	Exit Function
  End If
  ...
  Dim i
  EX.Pattern = "(\.\.\/)"'看有多少个../
  For i=0 To Ubound(spath) - 1 - EX.Execute(Matches(0).value).count
	  src = src & spath(i) & "/"
  Next
  '把../替换成正确的目录
...
on error resume next'容错支持
  ...
  '0=标签名,1=标签属性
  For Each Match in Matches
  	html = Replace(html,Match.value,eval(Trim(Match.SubMatches(0))))
  Next
  Set Matches = Nothing
Assign_asp = html
End Function
...
Private Function Assign_loop_bodytag(ByVal strCode)
  ...
  Set Matches = EX.Execute(strCode) '执行搜索
  If Matches.Count>0 Then'如果有<empty>标签
    For Each Match In Matches
      aryBody(0) = EX.Replace(strCode, "")'提取主体
...
Private Function Assign_loop(ByVal strCode)
  ...
  For Each Match in Matches
	  ...
	  If IsArray(BlockValue) Then'如果输入的是数组		
	    ...
	    For i = 0 To Ubound(rs,2)'循环输出记录
		  OutputTag = OutputTag & Assign_loop_Tag(block_index ,block_index ,strBlock(0) ,rs , i)'标签输出(块索引,循环内容,记录集,序号)
		  '嵌套循环支持
...
Dim rs,i,substr
  ...
  For Each Match in Matches'0=标签名,1=参数部分(如果没有就为空),2=循环体
	...
	blockid = block_index&"."&subLoop'索引规则是 上一级的 loopname.本级.loopname loop>loop
	'块的循环体'检测循环体中的<empty>标签,并且提取出来
	...
	If IsArray(arrAttr) Then'获得子循环的值并且替换输出
		...
		If TYPENAME(rs)="Variant()" Then'二维数据
				'循环输出记录集'由于是二维数组，没有记录到字段名，所以数据输出只能用下标
				For i = 0 To Ubound(rs,2)'循环输出记录
					substr = substr & Assign_loop_Tag(subLoop, blockid ,subBody ,rs , i)'标签输出(块内容名称,块索引,循环内容,记录集,序号)
...
Dim Match,Matches
  ...
  Set Matches = EX.Execute(" " & attrCode & " ") '执行搜索。

  If Matches.count Then
	 attr = Matches(0).SubMatches(1)
  End If
  Set Matches = Nothing
  GetAttribute = attr
End Function
...
Private Function Assign_loop_Tag(ByVal strTag ,ByVal strTagIndex ,ByVal strBody ,ByVal arrRs ,ByVal intRowsNum)
  ...
  For Each Match in Matches
	  ...
	  id = iif(IsNumeric(id), id, RS_FIELD(strTagIndex&"."&lcase(id)))
	  
	  '单标签的值
	  '如果下标超过字段宽度，那么这个值为空
	  If Len(id)>0 Then
	     id = Int(id)
	     OutputTag = iif( (id>Ubound(rs,1)),"",Return_Tag_Val(rs(id , intRowsNum),Match.SubMatches(2)))
	  End If
	  
	  html = Replace(html,Match.value,OutputTag)
  Next
  Set Matches = Nothing
Assign_loop_Tag = html'一条完整的循环内容返回
End Function
...
Private Function Analysis_block_loop(ByVal strCode)
  ...
  If TypeName(Matches)<>"IMatchCollection2" Then
	  Analysis_block_loop = strCode
	  Exit Function
  End If
  ...
  For Each Match in Matches
	...
	If IsArray(arrAttr) Then
      block_id = iif(Len(block_name),block_name,"block_" & j)
...
Private Function GetLoopBlock(ByVal strCode)
  Dim Match,Matches
  Dim Pattern : Pattern = "<loop(:[[a-zA-Z0-9]+)?\s*?([\s\S.]*?)>([\s\S.]*?)<\/loop\1>"
      EX.Pattern = Pattern
  Set Matches = EX.Execute(strCode) '执行搜索。
      If Matches.Count Then
	  Set GetLoopBlock = Matches
	  Else
	  Set GetLoopBlock = Nothing
	  End If
  Set Matches = Nothing
End Function
  ...
  Set ifNode = XMLDOM.SelectSingleNode("//template/if")'if有多少个节点
	  For i = 0 To ifNode.ChildNodes.Length - 1
		 ...
		 Set block_tpl = ifNode.ChildNodes(i).getElementsByTagName("tpl")
		   ...
		   For j = 0 To block_tpl.Length - 1
		     array_2(j) = block_tpl.item(j).text
		   Next
...
Private Function Analysis_SaveToXml(ByVal strPath)
  ...
  Dim keys,items,i
  Dim tplNode
  Dim blockNode,block_attr,block_code
  ...
  Analysis_SaveToXml = iif((Err.Number = 0),True,False)
End Function

'//----缓存模板
Private Sub FileCache()
	'缓存路径
	Dim strCache_Path : strCache_Path = strSITEROOT & strCache_dir & strTemplate_dir
	
	AutoCreateFolder(strCache_Path)'自动创建缓存文件目录
...
Public Property Get removeHTML(ByVal strHtml)
  ...
  EX.Global = True
  EX.Pattern = "<[^>]*>"
  removeHTML = EX.Replace(strHtml, "")
End Property

'清除所有的空格，空行等格式，只留文本
Public Property Get removeSpace(ByVal strHtml)
  EX.IgnoreCase = True
  EX.Global = True
  EX.Pattern = "[\n\s*\r|]"
  removeSpace = EX.Replace(strHtml, "")
End Property
'////缓存部分'////
'缓存到内存
...
Public Property Get Load()
  ...
  IF (intCacheflag = 1) Then'使用文件缓存
		...
		If FSO.FileExists(strCacheDir & "_.html") Then'如果能读取到缓存
			Dim Files : Set Files = FSO.GetFile(strCacheDir & "_.html")
			'检测文件缓存是否过期
			If DateDiff("n", FormatDateTime(Files.DateLastModified,0),Now())<intCacheTplTime Then'如果没有超时
				strTemplate = LoadFile( strCacheDir & "_.html" , strCharset)
			Else
				FileCacheTimeOut = true
			End If
		Else
			FileCacheTimeOut = true
		End If
...
Private Property Get strMeta
  strMeta = "<meta name=""copyright"" content="""&authorEmail&""">"
End Property

'//----清除所有模板缓存
Public Property Get clearFileCache
  Dim path : path = strSITEROOT & strCache_dir
      path = Replace(path,"/","\")
...
Public Property Let add(ByVal strTag,ByVal strVal)
  ...
  If Left(strTag,1)="@" Then
		 ...
		 If rs.State And not rs.eof Then
			 ...
			 For i=0 to rs.Fields.count-1
			    ...
			    If DIC_BLOCK_ATTR.Exists(strTag) Then DIC_BLOCK_ATTR.Remove(strTag)
				   DIC_BLOCK_ATTR(strTag) = iif(IsNull(rs(i)),"",rs(i))
			 Next
			'rs.Close()'程序后面还可以引用rs记录集，所以这里不用关闭，不过在外面的时候记得要关闭
		 End If
		 Set rs = Nothing
	 ...
	 ELSE
		 '否则就是一个一个的单标签赋值
		 If DIC_BLOCK_ATTR.Exists(strTag) Then DIC_BLOCK_ATTR.Remove(strTag)
			DIC_BLOCK_ATTR(strTag) = iif(IsNull(strVal),"",strVal)
		 Exit Property
	 End If
  End If
...
Public Property Let CachePageTimeout(ByVal strVal)
  ...
  If cbool(intCachePageTimeout) Then
	Dim CachePagePath : CachePagePath = "pagecache/"
	 ...
	 If FSO.FileExists(strCachePage) Then'如果没有缓存
	 	Dim Files
	 	Set Files = FSO.GetFile(strCachePage)
		'如果超时了
...
Public Property Let OutPutPage(ByVal strPath,ByVal strVar)
  Dim strOutputPath
  strOutputPath = strPath & "/" & iif(Len(strVar),strVar,"index.html")
