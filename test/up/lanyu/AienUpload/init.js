...
AjaxProcesser.prototype.succeed=function(a){return;};
AjaxProcesser.prototype.faild=function(a){return;};
AjaxProcesser.prototype.addFile=function(){
	if(this.FileCount>=this.MaxFileCount){alert("超过允许的最大文件上传数量；\n\n最多允许上传" + this.FileCount + "个文件");
		return;
	}
	_this=this;
	...
	file.size=40;
	if(!this.defaultStyle){file.style.cssText="font-size:9pt;border:1px #DDD solid;padding:3px 0px 1px 3px;height:20px;";}
	...
	remove.value="移除";
	remove.type="button";
	if(!this.defaultStyle){remove.style.cssText="font-size:9pt;border:1px #DDD solid;padding:3px 3px 1px 3px;height:20px;margin-left:3px;";}
	remove.onclick=function(){_this.frm.removeChild(this.previousSibling.previousSibling);
...
AjaxProcesser.prototype.createUploader=function(){
	...
	button.value="上传";
	button.type="button";
	if(!this.defaultStyle){button.style.cssText="margin-left:5px;font-size:9pt;border:1px #DDD solid;padding:3px 3px 1px 3px;height:20px;margin-top:3px;";}
	...
	if(this.MaxFileCount>1){
		...
		add.value="添加文件";
		add.type="button";
		if(!_this.defaultStyle){add.style.cssText="font-size:9pt;border:1px #DDD solid;padding:3px 3px 1px 3px;height:20px;margin-top:3px;margin-left:3px;";}
	...
	processDiv.style.cssText="display:none;padding:3px;font-size:9pt;border:1px #DDD solid;width:406px;margin:5px 2px 2px 0px;";
	...
	processDiv.appendChild(processIng);
	var processBar=document.createElement("div");
	processBar.style.cssText="font-size:9pt;width:400px;padding:0px;margin:0px;height:auto;border:1px #DDD solid;background-color:#eee;";
	var process=document.createElement("div");
	process.style.cssText="font-size:9pt;text-align:center;background-color:#AAA;width:0px;height:13px;padding:1px 0px 0px 2px;";
	this.process=process;
	processBar.appendChild(process);
	this.processBar=processBar;
	processDiv.appendChild(processBar);
...
AjaxProcesser.prototype.getProcess=function(){
	_this=this;
	Ajax({
		url:"Aienupload/getProcess.asp?processid=" + this.processID,
		method:"get",
		dataType:"json",
		asc:true,
