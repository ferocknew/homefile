...
(function($){
	...
	//定义主类并挂接到Jasp根下
    var streamfn = function(FilePath, v){
		this._stream = _stream;
    }
	$.extend({
        _stream: function(FilePath, v){
            return new streamfn(FilePath, v)
        }
    })
    $.extend(streamfn.prototype, {
		test:function(){
			
			Response.Write(typeof(this._stream));
		},
		test1:function(){}
    });
})(Jasp);
...
with(_stream){
	...
	Charset="utf-8";
	try {
		LoadFromFile(Server.MapPath("tpl.test.asp"));
	}
	catch(err){
