	...
	foreach ($sequence as $position) {
		...
		switch ($position) {
			...
			case 'attachment':
				...
				$img = suffusion_get_image_from_attachment($size, $original, $get_original, $attachment_id);
				continue;
			case 'embedded':
 ...
 */
function suffusion_get_image_from_attachment($size = 'thumbnail', &$original = array(), $get_original = false, $attachment_id = false) {
	global $post;
	$img = "";
	if (!$attachment_id) {
